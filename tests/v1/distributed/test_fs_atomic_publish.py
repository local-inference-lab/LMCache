...
def _native_store_process(
    ...
    try:
        ...
        try:
            ...
            completed_id, ok, error, results = _wait_for_native_completion(client)
            result_queue.put((completed_id == future_id, ok, error, results))
        finally:
            client.close()
...
def test_native_duplicate_workers_publish_one_value(tmp_path) -> None:
    ...
    try:
        ...
        assert ok, error
        assert results == [True] * len(payloads)
        assert (tmp_path / _object_key_to_filename(_key())).read_bytes() in payloads
        assert list(pending.iterdir()) == []
    finally:
        client.close()
...
def test_native_cross_process_writers_publish_one_value(tmp_path) -> None:
    ...
    try:
        ...
        assert all(submitted and ok for submitted, ok, _error, _bits in results), (
            results
        )
        assert all(bits == [True] for _submitted, _ok, _error, bits in results)
    finally:
        start_event.set()
        for process in processes:
