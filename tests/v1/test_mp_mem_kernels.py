...
def test_fused_nhd_transfer_roundtrip_odd_block_stride() -> None:
    """Scalar selection preserves byte-addressed dim-0 padding."""
    device = torch.device("cuda")
    nl, nb, bs, nh, hs = 1, 4, 1, 2, 16
    block_stride_elems = 33
    source_padding, target_padding = 251, 239
    source_pool = torch.full(
        (nb * block_stride_elems,), source_padding, dtype=torch.uint8, device=device
    )
    target_pool = torch.full(
        (nb * block_stride_elems,), target_padding, dtype=torch.uint8, device=device
    )
    for block_idx in range(nb):
        block_start = block_idx * block_stride_elems
        source_pool[block_start : block_start + nh * hs] = torch.arange(
            nh * hs, dtype=torch.uint8, device=device
        ) + (block_idx * 37)

    strides = (block_stride_elems, nh * hs, hs, 1)
    source = [source_pool.as_strided((nb, bs, nh, hs), strides)]
    target = [target_pool.as_strided((nb, bs, nh, hs), strides)]
    objects = create_memory_objects(
        1,
        nl,
        2,
    ...
    assert torch.equal(source[0][0], target[0][2])
    assert torch.equal(source[0][1], target[0][3])
    for block_idx in range(nb):
        padding_idx = (block_idx + 1) * block_stride_elems - 1
        assert source_pool[padding_idx].item() == source_padding
        assert target_pool[padding_idx].item() == target_padding
