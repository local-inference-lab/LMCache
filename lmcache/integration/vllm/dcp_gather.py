...
from lmcache.v1.storage_backend.disk_index_rebuild import sync_layout
...
def _packed_row_bytes(kvcaches: "list[torch.Tensor]") -> int:
    return sum(_flat(cache).shape[-1] * cache.element_size() for cache in kvcaches)


def dcp_local_token_count(global_token_count: int, world_size: int) -> int:
    return (global_token_count + world_size - 1) // world_size
...
def _extract_shard(
    ...
    for cache in kvcaches:
        selected = _flat(cache).index_select(0, rslots).contiguous()
        parts.append(selected.view(torch.uint8).view(selected.shape[0], -1))
    return torch.cat(parts, dim=1).unsqueeze(0).unsqueeze(0)


def _scatter_shard(
    kvcaches: "list[torch.Tensor]", rslots: torch.Tensor, packed: torch.Tensor
) -> None:
    rows = packed[0, 0]
    offset = 0
    for cache in kvcaches:
        flat = _flat(cache)
        row_bytes = flat.shape[-1] * cache.element_size()
        values = (
            rows[:, offset : offset + row_bytes]
            .contiguous()
            .view(cache.dtype)
            .view(len(rslots), flat.shape[-1])
        )
        flat.index_copy_(0, rslots, values)
        offset += row_bytes
    if offset != rows.shape[-1]:
        raise ValueError(f"Packed KV row has {rows.shape[-1]} bytes, consumed {offset}")
...
def _dcp_store(
    ...
    for start, end, key in engine.token_database.process_tokens(
        ...
        shard = _extract_shard(kvcaches, rslots)
        gathered = group.all_gather(shard, dim=2)
        ...
        # The allocation result is rank-0-only state, so it must never change the
        # number of all_gather calls this loop makes: leaving early here strands
        # ranks 1..N-1 in a collective that rank 0 will never join.
        if storage is not None and not cache_full:
            full = storage.allocate(
                torch.Size([1, 1, n, packed_bytes]),
                torch.uint8,
                fmt=engine.fmt,
                busy_loop=engine.config.get_extra_config_value(
            ...
            if full is None:
                ...
                full.tensor.copy_(_interleave(gathered, world, blk, n))
                keys.append(key)
                mobjs.append(full)
        n_chunks += 1
    if cache_full:
        logger.warning("DCP gather: CPU cache full, stored %d chunks", len(keys))
    ...
    for start, end, key in engine.token_database.process_tokens(toks, mask=msk):
        ...
        if int(flag.item()) == 0:
            if mobj is not None:
                mobj.ref_count_down()
            continue
        packed_gpu = torch.empty((1, 1, n, packed_bytes), dtype=torch.uint8, device=dev)
        if mobj is not None:
            packed_gpu.copy_(mobj.tensor)
            mobj.ref_count_down()
        tp_group.broadcast(packed_gpu, src=0)  # packed prefix KV to every TP rank
        rslots = slot_mapping[start // world : end // world]
        shard_packed = _deinterleave(packed_gpu, world, rank, blk, nl)
        _scatter_shard(kvcaches, rslots, shard_packed)
        loaded += n
    return loaded
