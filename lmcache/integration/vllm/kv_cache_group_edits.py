...
"""Normalize vLLM hybrid KV pages for byte-exact LMCache transfer.

vLLM scheduler block identifiers address logical cache pages, while an
attention backend can register smaller kernel pages or an opaque recurrent
state buffer. The rules in this module expose zero-copy tensor views whose
leading page dimension matches the scheduler identifier space.

The edits apply only to hybrid configurations that contain Mamba-compatible
recurrent layers. Unsupported cache specifications fail during registration
instead of entering a transfer path with incompatible addressing semantics.
"""
...
class _SubpagedMLAAttentionViewEdit(KVCacheGroupEdit):
    """Re-view kernel-paged MLA storage as scheduler-visible blocks.

    The MLA backend can expose ``[num_kernel_pages, kernel_block_size,
    head_size]`` while the scheduler addresses larger logical blocks. For
    Kimi-K3, twelve contiguous 64-token kernel pages form one 768-token
    scheduler block. The result is a zero-copy
    ``[num_logical_blocks, logical_block_size, head_size]`` view.
    """
    ...
    def apply(
        ...
        """Re-view a three-dimensional MLA tensor at logical-block granularity.
        ...
        """
        if not isinstance(kv_cache, torch.Tensor):
            raise ValueError("kernel-paged MLA cache must be a torch.Tensor")
        logical_block_size = spec.block_size
...
def apply_kv_cache_group_edits(
    ...
    Args:
        ...
        kv_caches: Registered tensors keyed by layer name. Recurrent entries
            are either unified page tensors or legacy ``[conv_state,
            ssm_state]`` lists; attention entries are single tensors.
