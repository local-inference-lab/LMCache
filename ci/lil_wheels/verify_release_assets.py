...
"""Verify immutable LMCache wheel release assets before publication or promotion."""
...
def verify_release(
    ...
    *,
    ...
    require(beta_tag == f"lmcache-cu134-sm120-beta-{source_commit}", "invalid beta tag")
    ...
    require(
        ...
        "release must contain exactly one lmcache package",
    )
    package = packages[0]
    require(package.get("name") == "lmcache", "release package must be lmcache")
    wheel_name = package.get("file", "")
    require(
        isinstance(wheel_name, str)
        and wheel_name.startswith("lmcache-")
        and wheel_name.endswith(".whl")
        and all(c.isascii() and (c.isalnum() or c in "_.+-") for c in wheel_name),
        "invalid wheel filename",
    )
    archive = f"lmcache-cu134-sm120-{source_commit}.tar.zst"
    ...
    receipt = json.loads((directory / "community-source.json").read_text())
    require(
        receipt == manifest.get("community_source"), "community source receipt mismatch"
    )
    require(
        receipt.get("source_commit") == source_commit, "receipt source commit mismatch"
    )
    require(
        receipt.get("schema") == "local-inference-lmcache-source-receipt/v1",
    ...
    if promotion:
        ...
        required_fields = {
            "schema": "local-inference-lmcache-promotion/v1",
